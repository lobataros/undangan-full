...
const formatDate = (date) => {
  ...
  const tanggal = new Intl.DateTimeFormat('id-ID', { dateStyle: 'full', timeStyle: 'short' }).format(convertDate);
    ...
    result += `
      <div class="card-body border-start bg-light py-2 ps-2 pe-0 my-2 ms-3 me-0" id="${data.uuid}">
          <div class="d-flex flex-wrap justify-content-between align-items-center">
              ...
              <small class="text-dark m-0 p-0" style="font-size: 0.75rem;">${formatDate(data.created_at)}</small>
          </div>
          <hr class="text-dark my-1">
